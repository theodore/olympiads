...
#ifdef DB
std::ostream& operator<<(std::ostream& o, const Vint& v)
{
    o << '{' << v[0];
...
#if defined(DB) && DB <= 1
	cerr << __func__ << endl;
	cerr << v->path.first << " + " << p.first << " =\n";
	db_err(path);
	db_err(vrtx);
	cerr << "  " << vrtx->path.first.back() << endl;
	cerr << endl;
#endif
    }
...
#ifdef DB
	    cerr << "Inserted: " << trie.lastins->path.second <<", ";
	    trie.output();
	    fprintf(stderr, "\nk = %d\n", k);
	    //return 0;
#endif
