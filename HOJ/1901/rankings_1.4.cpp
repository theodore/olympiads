 ...
 *  @param  x  Value.
 *  @param  pos  Postion of the root of the sub-tree.
 */
int idyllic::treap::equal(int x, int pos)
{
	if (pos == 0 || node[pos].data != x)
		return 0;
	return equal(x, node[pos].son[0]) + equal(x, node[pos].son[1]) + 1;
}
