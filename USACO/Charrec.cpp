...
LANG: C++
*/
// Fuck, in the sample, d is better than a, both have line 0 missing
				...
				for (int j=0; j<20; ++j)
					for (int k=0; k<20; ++k)
						if (font[i][j][k] != text[cur-20+j][k])
							++now;
				if (now+D(cur) < row[cur-20].dif[1]){
					row[cur-20].dif[1] = now + D(cur);
					row[cur-20].ans[1] = i;
					q.push(cur-20);
				}
			}
			...
			for (int i=0; i<27; ++i)
				for (int line=0; line<20; ++line){
					...
					for (int j=0; j<line; ++j)
						for (int k=0; k<20; ++k)
							if (font[i][j][k] != text[cur-21+j][k])
								++now;
					int u(0), v(0);
					for (int k=0; k<20; ++k){
						if (font[i][line][k] != text[cur-21+line][k])
							++u;
					...
					for (int j=line+1; j<20; ++j)
						for (int k=0; k<20; ++k)
							if (font[i][j][k] != text[cur-20+j][k])
								++now;
					if (now+D(cur) < row[cur-21].dif[2]){
						row[cur-21].dif[2] = now + D(cur);
						row[cur-21].ans[2] = i;
						q.push(cur-21);
					}
				}
		...
		if (cur >= 19)
			for (int i=0; i<27; ++i)
				for (int line=0; line<20; ++line){
					...
					if (now+D(cur) < row[cur-19].dif[0]){
						row[cur-19].dif[0] = now + D(cur);
						row[cur-19].ans[0] = i;
						q.push(cur-19);
					}
				}
