...
bool good(Point x){
//	cout << "Or problem here?" << endl;
//	x.output();printf("\n");
	for (int i=0; i<n; ++i)
		if (cross(eye, x, edge[i])){
//			cout << "We're corssing, ";
//			eye.output();
//			putchar(' ');
//			x.output();
...
//			edge[i].output();
	...
	//add: if a point is on the ray
	for (int i=0; i<n; ++i)
		if (!(point[i] == x) && on(i, x) && cross(eye, x, Edge(point[(i+1)%n], point[i?i-1:n-1])))
			return false;
	return true;
}
